...
当前布局分析
 ...
 基于 Supabase 的特性，以下是可以简化的部分：
  ...
  | 语法                                  | 效果                      |
  |-------------------------------------|-------------------------|
  | {npc.knowledge_scope.does_not_know} | 1. 凶手是谁2. 凶器在哪 （默认编号列表） |
  | {...|comma}                         | 凶手是谁, 凶器在哪              |
  | {...|bullet}                        | • 凶手是谁• 凶器在哪            |
  | {...|dash}                          | - 凶手是谁- 凶器在哪            |
  | {...|newline}                       | 凶手是谁凶器在哪                |
  ...
  更实用的方向是在现有功能中集成 AI 辅助，比如：

  1. 线索润色 - 对已有线索描述进行 AI 优化
  2. NPC 对话优化 - 润色 NPC 的背景故事、性格描述
  3. 触发条件建议 - AI 分析线索内容，建议合适的触发关键词
  4. 线索链检查 - AI 分析现有线索链的逻辑性，给出改进建议
