...
在对话模拟中，NPC 回复生成通常需要 5-15 秒。为了提升用户体验，NPC 回复支持流式（streaming）生成，让用户能够实时看到 token 逐字输出，而不是等待完整响应。

### 功能特性

1. **流式显示**
   - NPC 回复逐 token 显示，带闪烁光标动画
   - 回复生成过程中显示「生成中」状态标签
   - 支持中途停止生成（红色停止按钮）

2. **LLM 用量统计**
   - 流式模式下也能正确收集和显示 LLM 用量信息
   - 包含匹配（Matching）和 NPC 回复两部分的统计：
     - token 数量（prompt / completion / total）
     - 响应延迟（latency_ms）
     - 使用的模型（model）

3. **日志保存**
   - 流式生成的对话也会正确保存到对话日志
   - 日志详情页可查看完整的 LLM 用量统计

### 技术实现

#### API 端点

新增流式端点 `POST /api/simulate/stream`，返回 Server-Sent Events (SSE)：

```
event: match_result
data: {"matched_clues": [...], "triggered_clues": [...], "debug_info": {...}, "matching_llm_usage": {...}}

event: npc_chunk
data: {"chunk": "文本片段"}

event: complete
data: {"npc_response": "完整回复", "prompt_info": {...}, "npc_llm_usage": {...}, "log_id": "..."}
...
| 事件 | 说明 | 时机 |
|-----|------|-----|
| `match_result` | 线索匹配结果 | 匹配完成后立即发送（非流式） |
| `npc_chunk` | NPC 回复文本片段 | 每收到一个 token 发送一次 |
| `complete` | 完成信号 | 流结束时发送，包含最终数据和 log_id |
| `error` | 错误信息 | 发生错误时发送 |

#### LLM Usage 数据结构

```typescript
// 匹配 LLM 用量（在 match_result 事件中）
matching_llm_usage: {
  matching_tokens: {
...
1. **发送消息**
   ...
   - 发送后立即显示玩家消息气泡
   - 开始显示 NPC 消息占位符（加载骨架屏）

2. **流式显示**
   - 收到第一个 `npc_chunk` 后，替换骨架屏为实际内容
   - 后续 chunk 追加到消息内容中
   - 消息末尾显示闪烁光标表示正在生成

3. **停止生成**
   - 生成过程中显示红色停止按钮
   - 点击停止按钮取消请求（AbortController）
   - 已生成的内容保留显示

4. **完成状态**
   - 收到 `complete` 事件后，移除闪烁光标
   - 更新 LLM 用量统计面板（如果存在）
   - 保存对话到历史记录
...
| 场景 | 处理方式 |
|-----|---------|
| 用户取消生成 | AbortController 取消 fetch，清理流式状态，保留已生成内容 |
| LLM 超时 | 后端捕获 TimeoutException，返回 error 事件，前端显示错误提示 |
| 网络断开 | 前端 catch error，显示错误提示，清理流式状态 |
| NPC 回复未启用 | 不发送 npc_chunk 事件，直接返回 complete |
| 空响应 | complete 事件中 npc_response 为 null，不显示 NPC 消息气泡 |

### 向后兼容

- 原有的 `POST /api/simulate` 端点保持不变，返回完整 JSON 响应
- 流式端点 `POST /api/simulate/stream` 为新增端点
- 前端默认使用流式端点，如需要可配置回退到非流式
